...
// ===== Fechas: "DD/MM/AAAA HH:MM:ss.sss" =====
...
// ===== Ordenar mediciones (burbuja simple) =====
...
// ===== Umbrales / anomalías (P usa promedio de PS/PD) =====
int getUmbral(const Umbral* cfg, int n, const char* tipo, double& umin, double& umax){
    int i = 0;
    while (i < n) {
        // comparar hasta 6 chars sin strncmp
        int iguales = 1;
        int k = 0;
        while (k < 6) {
...
int esAnomalaLectura(const Lectura& L, const Umbral* cfg, int n){
    if (L.tipoSensor == 'E') return 0; // ECG no se clasifica aquí
    if (L.tipoSensor == 'T') {
        double a=0.0,b=0.0;
...
// ===== Recolección (sin realloc): 2 pasadas =====
...
void generarReportePaciente(const char* idPaciente,
    ...
    // --- PASO 2: reservar exacto y llenar (sin operador '?') ---
    ...
    if (nT>0) {
        ...
        while (i<nT) {
            ...
            sum += v;
            const char* estado = anom ? "ANOMALA" : "NORMAL";
